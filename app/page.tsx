...
const projects = [
  ...
  },
  {
    ...
    description:
      "Executed the consolidation of third-party media workflows from Singapore into our centralized, in-house cloud storage system (using SDVI). This effort streamlined scattered resources into one source of truth, optimizing media lifecycle management.",
    provider: "Cloud Provider: SDVI",
    competencies: [
      "Workflow Onboarding",
  ...
  },
  {
    ...
    description:
      "Managed and led a team of 20 technicians and 2 supervisors. Key responsibilities included end-to-end staffing: interviewing, hiring, developing comprehensive training curricula, setting up remote access during the pandemic, and providing ongoing support.",
    ...
    <nav className="container mx-auto flex justify-between items-center">
      ...
      <div className={`hidden md:flex space-x-6 ${config.colors.text} text-sm font-medium`}>
        {["Home", "Projects", "Skills", "About", "Contact"].map((item) => {
          ...
          if (item === "Projects") href = "#projects-in-a-professional-setting";
          else if (item === "About") href = "#about-me";
...
const Hero = () => (
  <section id="home" className={`py-24 md:py-32 ${config.colors.background} text-center`}>
    <div className="container mx-auto px-6">
      <h1 className="text-4xl md:text-6xl font-extrabold leading-tight text-white">{config.name}</h1>
...
const ProjectCard = ({ project }: { project: Project }) => (
  <div
    className={`p-6 rounded-lg shadow-2xl transition duration-300 ${config.colors.card} hover:shadow-teal-500/50 flex flex-col h-full`}
  >
    ...
    <div className="mt-4">
      ...
      <div className="flex flex-wrap gap-2 mt-2">
        {project.competencies.map((comp) => (
          <span
            key={comp}
            className="text-xs font-medium bg-gray-700 text-teal-300 px-3 py-1 rounded-full"
          >
            {comp}
          </span>
        ))}
    ...
    <div className="container mx-auto px-6 max-w-3xl">
      ...
      <div className={`p-8 rounded-lg ${config.colors.card} shadow-xl`}>
        ...
        <p className={`${config.colors.text} leading-relaxed text-lg mt-4`}>
          My expertise spans digital media formats, cloud computing strategy, and effective staff 
          management (including interviewing, training, and remote operations support). My goal is to 
          continually evolve my competencies in infrastructure resilience and leadership to drive 
          enterprise-level transformation. I am actively seeking opportunities where strategic 
          technical leadership can directly impact organizational scale and operational efficiency.
        </p>
      </div>
    </div>
  </section>
